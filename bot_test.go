...
import (
	...
	mhttp "github.com/alekseiapa/tg-client/http"
)
...
func (s *BotTestSuite) SetupSuite() {
	...
	httpmock.RegisterResponder("GET", "https://api.telegram.org/bot111/getMe",
		httpmock.NewStringResponder(200, `{"ok":true,"result":{"id":1,"first_name":"Micha","username":"bot"}}`))

	bot, err := NewBot("111")
	s.Equal(err, nil)
	s.Equal(bot.Me.FirstName, "Micha")
	s.Equal(bot.Me.ID, int64(1))
	s.Equal(bot.Me.Username, "bot")
